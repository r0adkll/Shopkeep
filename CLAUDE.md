...
- Phases 0–2 are complete (vault `Roadmap.md`). Current work: Phase 3 — Etsy connect: make `StorefrontAdapter`/`PlatformCapabilities` real, OAuth+PKCE connect flow, polling order ingestion with the SKU→secondary-string→remembered matching chain, and D17 listing sync (review-before-push diffs, drift detection, sold-out/renewal ceremony with fee surfaced, import-with-mapping). First tasks with a live key are the verification gates listed in vault `Etsy Integration.md`. Hard prerequisite: the approved Etsy API key.
